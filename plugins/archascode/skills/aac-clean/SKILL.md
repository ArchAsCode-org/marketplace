...
The verb is intentionally separate from `/aac-apply` (the reflexive render
loop) and `/aac-init` (additive toolchain setup): teardown gets its own
verb so the destructiveness is legible at the call site.
...
- `pyproject.toml`, `.venv/` — owned by `/aac-init`, untouched here.
...
user the next move is `/aac-apply` (render + close the loop) when they
want output again. Leave that for them to invoke.
