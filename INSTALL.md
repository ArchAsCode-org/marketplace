...
not your shell PATH. To find the installed wrapper, ask Claude Code to
run `which archascode` in a session; then, from your own terminal:
...
## 5. If `archascode` isn't found

Restart the Claude Code session first — plugin `bin/` directories load at
session start. If it's still missing, locate the installed wrapper and
make sure it's executable:

```sh
find ~/.claude/plugins -name archascode -type f
chmod +x <the-path-it-prints>
```

Then restart the Claude Code session again.
